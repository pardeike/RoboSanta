...
struct OpenAI: Think {
    ...
    func generateText(_ prompt: String, _ model: Model) async -> Answer? {
        ...
        do {
            ...
            print("\(Int(Date().timeIntervalSince1970 - start))s \(result.usage.input_tokens)->\(result.usage.output_tokens) (\(await kronor(result.usage)) SEK)")
